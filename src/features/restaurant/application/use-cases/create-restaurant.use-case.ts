...
import { createRestaurantFromApi } from "../services/restaurant-factory.service";

export class CreateRestaurantUseCase implements UseCase {
  constructor(private readonly restaurantRepository: RestaurantRespositoryPort) {}

  async execute(restaurantApi: RestaurantApi): Promise<Restaurant> {
    const restaurant = createRestaurantFromApi(restaurantApi);
    const createdRestaurantFromRepository = await this.restaurantRepository.create(restaurant);
    const restaurant = 
